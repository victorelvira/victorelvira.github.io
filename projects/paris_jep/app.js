...
function showSheet(p, groupIds = []) {
  ...
  $('#dialog-content').innerHTML = `<div class="detail-content">${meta(p)}<h2 class="detail-heading" id="dialog-title">${esc(p.Name)}</h2>${status(p)}${detailBody(p)}${others.length ? `<div class="group-links"><span>También en esta dirección</span>${others.map(other => `<button type="button" data-related="${other.id}">${esc(other.Name)}</button>`).join('')}</div>` : ''}</div>`;
...
export function readFilters() { return { priorities: [...document.querySelectorAll('[name=priority]:checked')].map(i => i.value), access: $('#access').value, day: $('[name=day]:checked').value, from: $('#from').value, to: $('#to').value, includeUnknown: $('#unknown').checked }; }
export function render() {
  ...
  const active = Number(!!f.priorities.length) + Number(!!f.access) + Number(f.day !== 'any') + Number(!!(f.from || f.to));
...
if(document.modelContext?.registerTool){
  ...
  try{Promise.resolve(document.modelContext.registerTool({name:'filter_places',title:'Filtrar lugares de París JEP',description:'Actualiza los filtros visibles y devuelve los lugares de la selección que coinciden. No confirma reservas ni disponibilidad.',inputSchema:{type:'object',properties:{priorities:{type:'array',items:{type:'string',enum:['⭐','◼','○']}},access:{type:'string',enum:['','🟢','🎟️','🟡','🔴','🟠','⚪']},day:{type:'string',enum:['any','Saturday','Sunday','both']},from:{type:'string'},to:{type:'string'},includeUnknown:{type:'boolean'}},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},execute(input){
    if(!input||typeof input!=='object'||Array.isArray(input))throw new Error('Se esperaba un objeto de filtros.');
    const allowed=['priorities','access','day','from','to','includeUnknown'];if(Object.keys(input).some(k=>!allowed.includes(k)))throw new Error('Filtro desconocido.');
    const f={...readFilters(),...input};
    if(!Array.isArray(f.priorities)||f.priorities.some(p=>!['⭐','◼','○'].includes(p))||!['','🟢','🎟️','🟡','🔴','🟠','⚪'].includes(f.access)||!['any','Saturday','Sunday','both'].includes(f.day)||typeof f.includeUnknown!=='boolean')throw new Error('Valor de filtro inválido.');
    ...
    document.querySelectorAll('[name=priority]').forEach(i=>i.checked=f.priorities.includes(i.value));document.querySelectorAll('[name=day]').forEach(i=>i.checked=i.value===f.day);$('#access').value=f.access;$('#from').value=f.from;$('#to').value=f.to;$('#unknown').checked=f.includeUnknown;
