...
function detailBody(p) {
  ...
  return `<p class="address">${esc(p.Address)}</p><div class="schedules"><div class="schedule"><strong>Sábado 19</strong>${schedule(p.Saturday)}</div><div class="schedule"><strong>Domingo 20</strong>${schedule(p.Sunday)}</div></div><div class="card-links">${source ? `<a href="${source}" target="_blank" rel="noopener noreferrer">${generic(source) ? 'Artículo general JEP ↗' : 'Fuente / programa ↗'}</a>` : ''}${booking ? `<a href="${booking}" target="_blank" rel="noopener noreferrer"><b>Reservar ↗</b></a>` : ''}<a href="https://www.google.com/search?q=${encodeURIComponent(p.Name + ' journées du patrimoine 2026')}" target="_blank" rel="noopener noreferrer">Buscar programa ↗</a>${route ? `<a href="${route}" target="_blank" rel="noopener noreferrer">Cómo llegar ↗</a>` : ''}</div>${p.Verified ? '' : '<p class="detail-location">Fuente: página índice, no verificada individualmente.</p>'}${p.Latitude ? (p.geocoding?.type === 'street' ? '<p class="detail-location">Ubicación aproximada en la vía.</p>' : '') : '<p class="location-pending">Ubicación por precisar · disponible en la lista.</p>'}`;
}
function card(p) {
  return `<details class="card" id="place-${p.id}" data-card="${p.id}"><summary>${meta(p)}<h3>${esc(p.Name)}</h3>${status(p)}${avail(p)}</summary><div class="card-body">${detailBody(p)}${p.Latitude ? `<button type="button" class="show-on-map" data-place="${p.id}">Ver en el mapa</button>` : ''}</div></details>`;
}
function showSheet(p, groupIds = []) {
  const others = groupIds.map(id => places.find(p => p.id === id)).filter(other => other && other.id !== p.id);
  $('#dialog-content').innerHTML = `<div class="detail-content">${meta(p)}<h2 class="detail-heading" id="dialog-title">${esc(p.Name)}</h2>${status(p)}${avail(p)}${detailBody(p)}${others.length ? `<div class="group-links"><span>También en esta dirección</span>${others.map(other => `<button type="button" data-related="${other.id}">${esc(other.Name)}</button>`).join('')}</div>` : ''}</div>`;
